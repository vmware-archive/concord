...
void TestsBuilder::retrieveExistingBlocksFromKVB() {
  ...
  for (int key = 0; key < NUMBER_OF_KEYS; key++) {
    SimpleKeyBlockIdPair simpleKIDPair(replyObj->items[key].simpleKey,
                                       request->readVersion);
...
bool TestsBuilder::lookForConflicts(BlockId readVersion,
                                    size_t numOfKeysInReadSet,
                                    SimpleKey *readKeysArray) {
  ...
  for (i = readVersion + 1; (i <= lastBlockId_) && !foundConflict; i++) {
    ...
    for (size_t a = 0; (a < numOfKeysInReadSet) && !foundConflict; a++) {
      ...
      for (int b = 0; (b < currBlock->numOfItems) && !foundConflict; b++) {
        if (memcmp(readKeysArray[a].key, items[b].simpleKey.key, KV_LEN) == 0)
          foundConflict = true;
      }
    }
  }
...
void TestsBuilder::addNewBlock(size_t numOfWrites, SimpleKV *writesKVArray) {
  ...
  for (size_t i = 0; i < numOfWrites; i++) {
    ...
    allKeysToValueMap_[SimpleKeyBlockIdPair(simpleKey, lastBlockId_)] =
        simpleValue;
  }
...
void TestsBuilder::createAndInsertRandomConditionalWrite() {
  ...
  if (lastBlockId_ > prevLastBlockId_ + CONFLICT_DISTANCE) {
    readVersion = 0;
    while (readVersion < prevLastBlockId_)
      readVersion = lastBlockId_ - (rand() % CONFLICT_DISTANCE);
  }
  ...
  auto *request =
      SimpleCondWriteRequest::alloc(numOfKeysInReadSet, numOfWrites);
  ...
  bool foundConflict =
      lookForConflicts(readVersion, numOfKeysInReadSet, readKeysArray);
...
void TestsBuilder::createAndInsertRandomRead() {
  ...
  if (prevLastBlockId_ == lastBlockId_) {
    ...
    while (readVersion <= prevLastBlockId_)
      readVersion = (rand() % (lastBlockId_ + 1));
  }
  ...
  for (size_t i = 0; i < numberOfKeysToRead; i++) {
    ...
    KeyBlockIdToValueMap::const_iterator it =
        allKeysToValueMap_.lower_bound(simpleKIDPair);
    if (it != allKeysToValueMap_.end() &&
        (request->readVersion >= it->first.blockId) &&
